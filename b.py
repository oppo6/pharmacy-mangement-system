...
# ===== picture icon =========
picture_icon = Image.open('pic-icon.png')
...
##db_con=connect(host="127.0.0.1", user="Rohit", passwd="Rohit@123", database="rohit")
##mycur=db_con.cursor()
...
##    mycur=db_con.cursor()
##
##    find_user = 'SELECT * FROM RegLog WHERE Email = %s and Password = %s'
##    mycur.execute(find_user, [(email_entry.get()), (password_entry1.get())])
##
...
##                         textvariable=email)
##    email_entry2.place(x=40, y=20, width=256, height=34)
...
##    email_label2.place(x=40, y=0)
     ...
     # ====== phone ====================
    phone_entry2 = Entry(win, fg="#a7a7a7", font=("yu gothic ui semibold", 12), highlightthickness=2,
                         textvariable=phone)
    phone_entry2.place(x=40, y=35, width=256, height=34)
    phone_entry2.config(highlightbackground="black", highlightcolor="black")
    phone_label2 = Label(win, text='• Phone', fg="#89898b", bg='#f8f8f8',
                         font=("yu gothic ui", 8, 'bold'))
    phone_label2.place(x=40, y=10)
    ...
    new_password_entry = Entry(win, fg="#a7a7a7", font=("yu gothic ui semibold", 12), show='•', highlightthickness=2,
                               textvariable=password)
    new_password_entry.place(x=40, y=95, width=256, height=34)
    new_password_entry.config(highlightbackground="black", highlightcolor="black")
    new_password_label = Label(win, text='• New Password', fg="#89898b", bg='#f8f8f8', font=("yu gothic ui", 8, 'bold'))
    new_password_label.place(x=40, y=70)

    # ====  Confirm Password ==================
    confirm_password_entry = Entry(win, fg="#a7a7a7", font=("yu gothic ui semibold", 11), show='•', highlightthickness=2
                                   , textvariable=confirmPassword)
    confirm_password_entry.place(x=40, y=155, width=256, height=34)
    ...
    confirm_password_label = Label(win, text='• Confirm Password', fg="#89898b", bg='#f8f8f8',
                                   font=("yu gothic ui", 8, 'bold'))
    confirm_password_label.place(x=40, y=130)

    otp2_entry = Entry(win, fg="#a7a7a7", font=("yu gothic ui semibold", 12), show='•', highlightthickness=2, textvariable=otp2)
    otp2_entry.place(x=40, y=215, width=256, height=34)
    otp2_entry.config(highlightbackground="black", highlightcolor="black")
    otp2_label = Label(win, text='• Confirm OTP', fg="#89898b", bg='#f8f8f8',font=("yu gothic ui", 8, 'bold'))
    otp2_label.place(x=40, y=190)
    ...
    update_pass.place(x=20, y=260, width=190, height=50)
    update_pass2 = Button(win, fg='#f8f8f8', text='SEND OTP', bg='#1b87d2', font=("yu gothic ui bold", 14),
                         cursor='hand2', activebackground='#1b87d2', command=lambda: otpgen())
    update_pass2.place(x=225, y=260, width=130, height=50)
    ...
    def otpgen():
            ...
            b=z+f
##            b=phone_entry2.get()
            pywhatkit.sendwhatmsg_instantly(phone_no=b, message=a)
    ...
    def change_password():
        global a
        if ((new_password_entry.get() == confirm_password_entry.get())and( a==otp2_entry.get())):
            db = sqlite3.connect("RegLog.db")
            curs = db.cursor()

            insert = '''update RegLog set Password=?, ConfirmPassword=? where phone= ? '''
            curs.execute(insert, [new_password_entry.get(), confirm_password_entry.get(),  phone_entry2.get() ])
            db.commit()
            db.close()
...
##        if new_password_entry.get() == confirm_password_entry.get()and a==otp2.get():
##            db_con=connect(host="127.0.0.1", user="Rohit", passwd="Rohit@123", database="rohit")
##            mycur=db_con.cursor()
##
##            insert = "update RegLog set Password=%s, ConfirmPassword=%s where phone=%s"
##            mycur.execute(insert, [new_password_entry.get(), confirm_password_entry.get(), phone_entry2.get()])
##            db_con.commit()
##            db_con.close()
##            messagebox.showinfo('Congrats', 'Password changed successfully')
##                
...
phone_entry = Entry(design_frame8, fg="#a7a7a7", font=("yu gothic ui semibold", 12), highlightthickness=2,
                    textvariable=phone)
phone_entry.place(x=284, y=450, width=286, height=34)
...
signUp2.place(x=280, y=500, width=286, height=50)
...
confirmPassword_icon = Image.open('pass-icon.png')
...
# ===== Full Name icon =========
name_icon = Image.open('name-icon.png')
...
##db_con=connect(host="127.0.0.1", user="Rohit", passwd="Rohit@123", database="rohit")
##mycur=db_con.cursor()
##mycur.execute("CREATE TABLE IF NOT EXISTS RegLog(Email TEXT PRIMARY KEY, FullName TEXT, Password TEXT,"
...
cur.execute("CREATE TABLE IF NOT EXISTS RegLog(Email TEXT PRIMARY KEY, FullName TEXT, Password TEXT,ConfirmPassword TEXT,phone TEXT)")
connection.commit()
connection.close()
    ...
    if check_counter == 6:
        try:
            ...
            cur.execute("INSERT INTO RegLog values(?,?,?,?,?)",
                        (Email.get(), FullName.get(), Password.get(), ConfirmPassword.get(),phone.get()))
